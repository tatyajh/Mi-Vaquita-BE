...
export async function getCalendarEvents(userId, { start='1900-01-01', end='2999-12-31' }={}) {
  ...
  const loans=(await pool.query(`SELECT l.id,l.natillera_id,n.name,l.issued_on,l.term_months,l.principal,l.interest,COALESCE(SUM(p.amount),0) repaid FROM NatilleraLoans l JOIN Natilleras n ON n.id=l.natillera_id LEFT JOIN NatilleraLoanPayments p ON p.loan_id=l.id WHERE l.user_id=$1 GROUP BY l.id,n.name`,[userId])).rows.map(x=>{const date=addMonths(x.issued_on,x.term_months);return {id:`loan:${x.id}`,type:'loan',date,title:`Vencimiento de préstamo · ${x.name}`,amount:Math.max(0,Number(x.principal)+Number(x.interest)-Number(x.repaid)),status:Number(x.repaid)>=Number(x.principal)+Number(x.interest)?'completed':date<todayInBogota()?'overdue':'pending',link:`/natilleras/${x.natillera_id}`};}).filter(x=>x.date>=start&&x.date<=end);
