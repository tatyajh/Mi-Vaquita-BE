...
const queries = [
  ...
  `CREATE TABLE IF NOT EXISTS UserSessions (id BIGSERIAL PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES Users(id) ON DELETE CASCADE,token_hash VARCHAR(64) NOT NULL UNIQUE,expires_at TIMESTAMP NOT NULL,revoked_at TIMESTAMP,last_used_at TIMESTAMP,user_agent VARCHAR(300),ip_address VARCHAR(64),created_at TIMESTAMP NOT NULL DEFAULT NOW());`,
  `CREATE INDEX IF NOT EXISTS user_sessions_active_idx ON UserSessions(user_id,expires_at) WHERE revoked_at IS NULL;`,
  `CREATE TABLE IF NOT EXISTS LegalInterestRates (id SERIAL PRIMARY KEY, valid_from DATE NOT NULL, valid_to DATE NOT NULL, annual_effective_rate NUMERIC(8,4) NOT NULL CHECK(annual_effective_rate >= 0), source_name VARCHAR(180) NOT NULL, source_url TEXT, created_at TIMESTAMP NOT NULL DEFAULT NOW(), UNIQUE(valid_from,valid_to), CHECK(valid_to >= valid_from));`,
  `INSERT INTO LegalInterestRates(valid_from,valid_to,annual_effective_rate,source_name,source_url) VALUES('2026-09-01','2026-09-30',29.2400,'Superintendencia Financiera de Colombia','https://www.superfinanciera.gov.co/') ON CONFLICT(valid_from,valid_to) DO NOTHING;`,
  ...
  `ALTER TABLE NatilleraLoanPayments ADD COLUMN IF NOT EXISTS capital_amount NUMERIC(12,2);`,
  `ALTER TABLE NatilleraLoanPayments ADD COLUMN IF NOT EXISTS interest_amount NUMERIC(12,2);`,
  `CREATE TABLE IF NOT EXISTS NatilleraLoanInstallments (id SERIAL PRIMARY KEY, loan_id INTEGER NOT NULL REFERENCES NatilleraLoans(id) ON DELETE CASCADE, installment_number INTEGER NOT NULL, due_on DATE NOT NULL, principal_due NUMERIC(12,2) NOT NULL CHECK(principal_due >= 0), interest_due NUMERIC(12,2) NOT NULL CHECK(interest_due >= 0), UNIQUE(loan_id,installment_number));`,
  ...
  `ALTER TABLE Invitations ADD COLUMN IF NOT EXISTS failed_pin_attempts INTEGER NOT NULL DEFAULT 0;`,
  `ALTER TABLE Invitations ADD COLUMN IF NOT EXISTS locked_until TIMESTAMP;`,
  ...
  `ALTER TABLE Notifications ADD CONSTRAINT notifications_status_check CHECK(status IN ('pending','prepared','sent','failed','manual_sent'));`,
